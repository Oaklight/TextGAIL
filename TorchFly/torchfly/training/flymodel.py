...
class FlyModel(nn.Module):
    ...
    def configure_optimizers(self, total_num_update_steps) -> [List, List]:
        ...
        betas = self.config.training.optimization.betas if hasattr(self.config.training.optimization, "betas") else (0.9, 0.999)
        ...
        warmup_cycle = self.config.training.scheduler.warmup_cosine_cycle if hasattr(self.config.training.scheduler, "warmup_cosine_cycle") else None
